...
try:
    ...
    lab.awg.set_default_params("1", length=20*us, amp=1 , freq=50*MHz+566)
    
    params.bin_length.value = 150*ms
    params.phase_start.value = "X"
    
    params.loops.value = 1
    # params.loops.value = np.arange(2, 100, 1)
    
    # params.tau.value = 20*ms
    params.tau.value = np.arange(0, 2, 100*ms)
    ...
    send_email("nmr", "laurent.bergeron4@gmail.com")
