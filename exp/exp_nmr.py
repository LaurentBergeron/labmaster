...
def update_plot(fig, params, data):
    ...
    ### 1D with phase cycling
        ...
        if params.phase_cycle.v=="-":
            ...
            size_ar = np.min(np.sum(np.isfinite(data)))
            cycle1 = data[:size_ar,0]
            cycle2 = data[:size_ar,1]
            cyclediff = cycle1 - cycle2
            plotting.updatefig_XY(fig, params.time_axis.value[:size_ar], cycle1, line_index=0)
            plotting.updatefig_XY(fig, params.time_axis.value[:size_ar], cycle2, line_index=1)
            plotting.updatefig_XY(fig, params.time_axis.value[:size_ar], cyclediff, line_index=2)
    ### no phase cycling
    else: 
