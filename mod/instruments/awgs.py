...
class Awg_M8190A(Instrument):
    ...
    def __init__(self, name, parent, resource):
        ...
        self.default_amp = {}
        self.default_offset = {}
        self.default_shape = {} ## the shape of the envelope
        self.set_default_params(for_channel="1", delay=0, length=0, freq=0, phase=0, amp=1.0, offset=0.0, shape="square")
        self.set_default_params(for_channel="2", delay=0, length=0, freq=0, phase=0, amp=1.0, offset=0.0, shape="square")
        # awg characteristics
        self.granularity = 64
        ...
        # nyquist sampling values for continous waveform (cw)
        self.nyquist_threshold = 4  ## Min factor between frequency and sample rate.
        ...
        try:
            ...
            for channel in ("1", "2"):
                ...
                self.set_amplitude(self.default_amp[channel], channel=channel)
                self.set_offset(self.default_offset[channel], channel=channel)
    ...
    def set_default_params(self, for_channel=None, delay=None, length=None, freq=None, phase=None, amp=None, offset=None, shape=None):
        ...
        if offset!=None:
            self.default_offset[channel] = offset
        if shape!=None:
